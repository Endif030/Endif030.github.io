...
family_jokes = [
    ...
    },
    {
        "id": "1.2", 
        "title": "Kevin家的健康争吵",
        "text": "当你还是个孩子的时候，你不会意识到你的家庭 messed up，因为那就是你所知道的一切。但然后会有一个时刻，你意识到一切都不太对劲。对我来说，那是七年级。我在朋友 Kevin 家过夜。Kevin 的爸爸走进客厅说：嘿，今晚谁想看《加勒比海盗》？然后 Kevin 的妈妈跑进来，说：绝对不行，那对他们来说太成熟了。Kevin 爸爸说：但我真的很想看《加勒比海盗》。Kevin 妈妈说：我想你会没事的 Kirk。然后 Kevin 说：我很抱歉让你看到这一幕。\n\n那是我听过的最健康的争吵。"
    },
    {
        "id": "1.3",
        "title": "父母7天大离婚",
    ...
    },
    {
        "id": "1.5",
        "title": "36楼烂地基",
        "text": "我现在36岁了。很多人都觉得，到了36岁，你应该停止把你所有的问题都归咎于父母了。好吧，让我问你这个问题：如果你在一栋地基烂掉的楼里，你觉得在36层会更安全吗？\n\n是的，我也不这么认为。"
    },
    {
        "id": "1.6",
        "title": "ex-stepdad的尴尬命名",
        "text": "上个月我和前继父一起吃饭。这种关系没有一个方便的称呼，因为它本不应该存在。我不知道哪个更好：前继父，还是那个下台的父亲。\n\n我敢肯定德语里一定有个词来形容一个曾经操我妈的男人。"
    },
    {
        "id": "1.7",
    ...
    },
    {
        "id": "1.8",
        "title": "给继父的婚姻建议",
        "text": "我记得有一次在高中，我继父开车送我。他们肯定刚吵过架什么的，因为他问我：我怎么才能让你妈妈开心？但在那个年龄，我对女人的唯一了解来自互联网。所以我说：我不知道。你试过用舌头拼字母表吗？\n\n我延长了那段婚姻六年。"
    },
    {
        "id": "1.9",
        "title": "妈妈在eHarmony",
    ...
    },
    {
        "id": "1.23",
        "title": "妈妈当瑜伽老师反击",
        "text": "当我对事业感到沮丧时，我妈会对我说：你是个演员。你为什么不演得不那么沮丧呢？\n\n所以我说：妈，你是个瑜伽老师。你为什么不自己去操自己呢？"
    },
    {
        "id": "1.24",
        "title": "妈妈握有黑料",
    ...
    },
    {
        "id": "1.29",
        "title": "意大利爸爸的家训",
        "text": "我爸是意大利人。所以他总是告诉我：儿子，记住，la famiglia è tutto。家庭就是一切。Male tua madre è una puttana。\n\n我现在还是不知道第二部分是什么意思，但听起来真好听。"
    },
    {
        "id": "1.30",
        "title": "爸爸心脏病-遗嘱事件",
...
# 创建图形 - 更大的画布，垂直排列
fig, ax = plt.subplots(1, 1, figsize=(24, 120))
ax.set_xlim(0, 100)
ax.set_ylim(0, 240)
ax.axis('off')
...
# 标题
title_y = 238
ax.text(50, title_y, "【原生家庭】主题笑点详解", fontsize=28, fontweight='bold', ha='center', va='top', color='#2C3E50')
ax.text(50, title_y - 3, "Tom Segura - Thief of Joy | 共34个笑点", fontsize=16, ha='center', va='top', color='#555')
ax.text(50, title_y - 6, "每个模块包含完整中文译文 | 实线箭头表示承接关系", fontsize=12, ha='center', va='top', color='#777', style='italic')
...
for i, joke in enumerate(family_jokes):
    ...
    # 计算文本高度（根据文本长度）
    text_lines = joke['text'].count('\n') + 2
    adjusted_height = max(box_height, text_lines * 0.6 + 1.5)
    
    # 绘制圆角矩形
    rect = FancyBboxPatch((x_center - box_width/2, y_pos - adjusted_height), box_width, adjusted_height,
                           boxstyle="round,pad=0.2",
                           facecolor='#7EB5A6', edgecolor='#2C3E50', 
                           linewidth=2, alpha=0.9)
    ax.add_patch(rect)
    
    # 编号和标题
    ax.text(x_center - box_width/2 + 1, y_pos - 0.5, f"{joke['id']} {joke['title']}", 
            fontsize=11, fontweight='bold', ha='left', va='top', color='#1A252F')
    
    # 文本内容
    ax.text(x_center, y_pos - 1.8, joke['text'], fontsize=9,
            ha='center', va='top', color='#2C3E50', wrap=True,
            linespacing=1.3, bbox=dict(boxstyle='round', facecolor='white', alpha=0.3, edgecolor='none'))
...
for i in range(len(box_positions) - 1):
    ...
    # 计算准确的连接点
    y1_bottom = y_start - i * spacing_y - box_height/2 - 0.5
    y2_top = y_start - (i+1) * spacing_y + box_height/2 + 0.5
...
# 标注callback关系
# 1.17 糖果 -> 1.18 万圣节
ax.annotate('', xy=(x_center + box_width/2 - 5, y_start - 17 * spacing_y + 2), 
           xytext=(x_center + box_width/2 - 5, y_start - 16 * spacing_y - 2),
           arrowprops=dict(arrowstyle='->', color='#E74C3C', lw=3, ls='--',
                         connectionstyle="arc3,rad=0.3"))
ax.text(x_center + box_width/2 + 2, y_start - 16.5 * spacing_y, 'callback', 
        fontsize=9, color='#E74C3C', fontweight='bold', rotation=90, va='center')

# 添加说明
ax.text(50, 2, "—— 实线箭头：承接关系  |  - - - 红色虚线：Callback 呼应", 
        fontsize=14, ha='center', style='italic', color='#555')
ax.text(50, 0.5, "注：每个模块包含该笑点的完整中文译文", fontsize=11, ha='center', color='#777')

plt.tight_layout()
plt.savefig('/root/.openclaw/workspace/transcripts/Thief_of_Joy_原生家庭详解.png', 
