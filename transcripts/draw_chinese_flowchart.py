...
for i, name in enumerate(legend_items):
    ...
    ax.add_patch(rect)
    ax.text(19 + i*13.5, legend_y-0.2, name, fontsize=10, ha='left', va='center')
...
for col_idx, theme in enumerate(columns):
    ...
    for i in range(len(jokes) - 1):
        ...
        ax.annotate('', xy=(x + box_width/2, y2), 
                   ...
                   arrowprops=dict(arrowstyle='->', color='#555', lw=1, alpha=0.6))
...
plt.tight_layout()
plt.savefig('/root/.openclaw/workspace/transcripts/Thief_of_Joy_中文流程图.png', 
